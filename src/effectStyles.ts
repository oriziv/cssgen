    ...
    localEffectStyles.forEach((style: EffectStyle) => {
        ...
        visibleEffects.forEach(e => {
            ...
            if (type === 'DROP_SHADOW' || type === 'INNER_SHADOW') {
                ...
                // Count styles
                const strVal = `${type === 'INNER_SHADOW' ? 'inset ' : ''}${formatNumericValue(effect.offset.x, 'px', 2)} ${formatNumericValue(effect.offset.y, 'px', 2)} ${formatNumericValue(effect.radius, 'px', 2)}${effect.spread ? ' ' + formatNumericValue(effect.spread, 'px', 2) : ''} ${val}`;
            ...
            if (type === 'BACKGROUND_BLUR' || type === 'LAYER_BLUR') {
                ...
                // Count styles
                textValues[`${type === 'BACKGROUND_BLUR' ? 'backdrop-filter ' : 'filter'}`] = `blur(${formatNumericValue(effect.radius, 'px', 2)})`;
